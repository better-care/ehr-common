...
class KotlinSerializationPolymorphicGenerator(private val codeGenerator: CodeGenerator) : SymbolProcessor {
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        resolver
            ...
            .forEach { annotated ->
                ...
                val reflections = Reflections(allowedPackages)

                val classHierarchy: ClassHierarchy = buildClassHierarchy(
                    reflections,
    ...
    private fun buildClassHierarchy(reflections: Reflections, parent: KClass<*>): ClassHierarchy {
        val children: List<ClassHierarchy> = reflections.getSubTypesOf(parent.java)
            .filter { it.superclass == parent.java }
            .map { buildClassHierarchy(reflections, it.kotlin) }
