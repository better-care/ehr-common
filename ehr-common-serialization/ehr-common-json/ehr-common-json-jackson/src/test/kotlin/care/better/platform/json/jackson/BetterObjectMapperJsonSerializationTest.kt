...
import care.better.platform.json.kotlin.serialization.better.BetterKotlinJsonSerializer
...
class BetterObjectMapperJsonSerializationTest {
    ...
    @Test
    fun testSimple() {
        val composition: Composition = buildComposition().checkCompatibilityBetweenJacksonAndKotlinSerializations()
        val compositionJson = objectMapper.writeValueAsString(composition)
        val compositionFromJson: Composition = objectMapper.readValue(compositionJson)
        assertThat(composition).usingRecursiveComparison().isEqualTo(compositionFromJson)
        val cluster: Cluster = buildCluster().checkCompatibilityBetweenJacksonAndKotlinSerializations()
        val clusterJson = objectMapper.writeValueAsString(cluster)
        val clusterFromJson: Cluster = objectMapper.readValue(clusterJson)
    ...
    fun teseInArray() {
        ...
        compositions.checkCompatibilityBetweenJacksonAndKotlinSerializations()
    ...
    fun testInArrayMultipleClasses() {
        ...
        val fromJson: List<Any> = objectMapper.readValue(jsonString)
    ...
    fun testSimpleMap() {
        ...
        val fromJson1: Map<String, Any?> = objectMapper.readValue(objectMapper.writeValueAsString(map))
        assertThat(fromJson1).usingRecursiveComparison().isEqualTo(map)
        val fromJson2: List<Any> = objectMapper.readValue(jsonString)
    ...
    fun testSimpleMapInList1() {
        val fromJson: List<Any> = objectMapper.readValue(
                """[
                          {
                            "tags": [
                              {
    ...
    fun testInMapMultipleClasses() {
        ...
        val fromJson: Map<String, Any?> = objectMapper.readValue(jsonString)
    ...
    fun testResultSetWithTags() {
        val fromJson: List<*> = objectMapper.readValue(BetterObjectMapperJsonSerializationTest::class.java.getResource("/simple.json"))
    ...
    fun testWrappedResultSetWithTags() {
        val fromJson: Result = objectMapper.readValue(BetterObjectMapperJsonSerializationTest::class.java.getResource("/result.json"))
        assertThat(fromJson).isNotNull
    ...
    fun rmAwareTest() {
        ...
        val reconstructedMap: Map<*, *> = openEhrMapper.readValue(openEhrMapJson)
        ...
        val reconstructedList: List<*> = openEhrMapper.readValue(openEhrListJson)
        ...
        val reconstructedSet: Set<*> = openEhrMapper.readValue(openEhrSetJson)
    ...
    fun dvIntervalDeserializationTest() {
        val interval: DvInterval = objectMapper.readValue(
    ...
    private inline fun <reified T : RmObject> T.checkCompatibilityBetweenJacksonAndKotlinSerializations(): T {
        val rmObjectJson = objectMapper.writeValueAsString(this)
        val rmObjectJsonUsingKotlinSerialization = BetterKotlinJsonSerializer().json.encodeToString<T>(this)
        val rmObjectFromJson: T = objectMapper.readValue(rmObjectJsonUsingKotlinSerialization)
        val rmObjectFromJsonUsingKotlinSerialization: T = BetterKotlinJsonSerializer().json.decodeFromString(rmObjectJson)
    ...
    private inline fun <reified T : RmObject> List<T>.checkCompatibilityBetweenJacksonAndKotlinSerializations(): List<T> {
        val rmObjectJson = objectMapper.writeValueAsString(this)
        val rmObjectJsonUsingKotlinSerialization = BetterKotlinJsonSerializer().json.encodeToString<List<T>>(this)
        val rmObjectFromJson: List<T> = objectMapper.readValue(rmObjectJsonUsingKotlinSerialization)
        val rmObjectFromJsonUsingKotlinSerialization: List<T> = BetterKotlinJsonSerializer().json.decodeFromString(rmObjectJson)
