...
class OpenEhrObjectMapperSerializationTest {
    ...
    fun testWrappedResultSetWithTags() {
        val fromJson: Result =
            objectMapper.readValue(OpenEhrObjectMapperSerializationTest::class.java.getResource("/openehr-result.json"))
        assertThat(fromJson).isNotNull()
    ...
    private inline fun <reified T : RmObject> List<T>.checkCompatibilityBetweenJacksonAndKotlinSerializations(): List<T> {
        val rmObjectJson = objectMapper.writeValueAsString(this)
        val rmObjectJsonUsingKotlinSerialization = OpenEhrKotlinJsonSerializer().json.encodeToString<List<T>>(this)
        val rmObjectFromJson: List<T> = objectMapper.readValue(rmObjectJsonUsingKotlinSerialization)
        val rmObjectFromJsonUsingKotlinSerialization: List<T> = OpenEhrKotlinJsonSerializer().json.decodeFromString(rmObjectJson)
